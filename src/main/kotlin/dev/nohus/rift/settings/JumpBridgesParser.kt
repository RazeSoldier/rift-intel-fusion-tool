...
class JumpBridgesParser(
    ...
    suspend fun parse(text: String): List<JumpBridgeConnection>? = withContext(Dispatchers.Default) {
    ...
    private fun parseGoonswarmWikiJumpBridges(text: String): List<JumpBridgeConnection>? {
    ...
    private fun parseAnyJumpBridges(text: String): List<JumpBridgeConnection>? {
        val systems = solarSystemsRepository.getSovSystems().map { it.name }
